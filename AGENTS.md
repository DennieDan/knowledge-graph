...
- **API environment variables (Render `kg-api`).** Prompted once at Blueprint
  ...
  - `WAHA_WEBHOOK_URL` — optional; blank means poll-only
...
- **Database.** Enable the `vector` extension in Supabase. Migrations run
  automatically as the Render API pre-deploy step; never point the local
  `apps/api/.env` at production.
